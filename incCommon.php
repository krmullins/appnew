		...
		$pcConfig = [
			...
			'employees' => [
				'ReportsTo' => [
					...
					'tab-label' => 'Subordinates <span class="hidden child-label-employees child-field-caption">(ReportsTo)</span>',
					'auto-close' => false,
			...
			'orders' => [
				'CustomerID' => [
					...
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => [0 => 'Order ID', 1 => 'Status', 2 => 'Customer', 3 => 'Employee', 4 => 'Order Date', 7 => 'Shipped Date', 8 => 'Ship Via', 15 => 'Ship Country', 18 => 'Total'],
					'display-field-names' => [0 => 'OrderID', 1 => 'Status', 2 => 'CustomerID', 3 => 'EmployeeID', 4 => 'OrderDate', 7 => 'ShippedDate', 8 => 'ShipVia', 15 => 'ShipCountry', 18 => 'Total'],
					'sortable-fields' => [0 => '`orders`.`OrderID`', 1 => 2, 2 => '`customers1`.`CompanyName`', 3 => 4, 4 => '`orders`.`OrderDate`', 5 => '`orders`.`OrderTime`', 6 => '`orders`.`RequiredDate`', 7 => '`orders`.`ShippedDate`', 8 => '`shippers1`.`CompanyName`', 9 => '`orders`.`Freight`', 10 => '`customers1`.`CompanyName`', 11 => '`customers1`.`Address`', 12 => '`customers1`.`City`', 13 => '`customers1`.`Region`', 14 => '`customers1`.`PostalCode`', 15 => '`customers1`.`Country`', 16 => 17, 17 => '`orders`.`added_date`', 18 => '`orders`.`Total`'],
					'records-per-page' => 10,
					'default-sort-by' => 0,
					...
					'query' => "SELECT `orders`.`OrderID` as 'OrderID', `orders`.`Status` as 'Status', IF(    CHAR_LENGTH(`customers1`.`CompanyName`), CONCAT_WS('',   `customers1`.`CompanyName`), '') as 'CustomerID', IF(    CHAR_LENGTH(`employees1`.`LastName`) || CHAR_LENGTH(`employees1`.`FirstName`), CONCAT_WS('',   `employees1`.`LastName`, ', ', `employees1`.`FirstName`), '') as 'EmployeeID', if(`orders`.`OrderDate`,date_format(`orders`.`OrderDate`,'%m/%d/%Y'),'') as 'OrderDate', TIME_FORMAT(`orders`.`OrderTime`, '%r') as 'OrderTime', if(`orders`.`RequiredDate`,date_format(`orders`.`RequiredDate`,'%m/%d/%Y'),'') as 'RequiredDate', if(`orders`.`ShippedDate`,date_format(`orders`.`ShippedDate`,'%m/%d/%Y'),'') as 'ShippedDate', IF(    CHAR_LENGTH(`shippers1`.`CompanyName`), CONCAT_WS('',   `shippers1`.`CompanyName`), '') as 'ShipVia', `orders`.`Freight` as 'Freight', IF(    CHAR_LENGTH(`customers1`.`CompanyName`), CONCAT_WS('',   `customers1`.`CompanyName`), '') as 'ShipName', IF(    CHAR_LENGTH(`customers1`.`Address`), CONCAT_WS('',   `customers1`.`Address`), '') as 'ShipAddress', IF(    CHAR_LENGTH(`customers1`.`City`), CONCAT_WS('',   `customers1`.`City`), '') as 'ShipCity', IF(    CHAR_LENGTH(`customers1`.`Region`), CONCAT_WS('',   `customers1`.`Region`), '') as 'ShipRegion', IF(    CHAR_LENGTH(`customers1`.`PostalCode`), CONCAT_WS('',   `customers1`.`PostalCode`), '') as 'ShipPostalCode', IF(    CHAR_LENGTH(`customers1`.`Country`), CONCAT_WS('',   `customers1`.`Country`), '') as 'ShipCountry', `orders`.`added_by` as 'added_by', if(`orders`.`added_date`,date_format(`orders`.`added_date`,'%m/%d/%Y'),'') as 'added_date', `orders`.`Total` as 'Total' FROM `orders` LEFT JOIN `customers` as customers1 ON `customers1`.`CustomerID`=`orders`.`CustomerID` LEFT JOIN `employees` as employees1 ON `employees1`.`EmployeeID`=`orders`.`EmployeeID` LEFT JOIN `shippers` as shippers1 ON `shippers1`.`ShipperID`=`orders`.`ShipVia` "
				],
				'EmployeeID' => [
					'parent-table' => 'employees',
					'parent-primary-key' => 'EmployeeID',
			...
			'order_details' => [
				'OrderID' => [
					...
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => [1 => 'Order ID', 2 => 'Category', 3 => 'Product', 4 => 'Unit Price', 5 => 'Quantity', 6 => 'Discount', 7 => 'Subtotal'],
					'display-field-names' => [1 => 'OrderID', 2 => 'Category', 3 => 'ProductID', 4 => 'UnitPrice', 5 => 'Quantity', 6 => 'Discount', 7 => 'Subtotal'],
					'sortable-fields' => [0 => '`order_details`.`odID`', 1 => '`orders1`.`OrderID`', 2 => 3, 3 => '`products1`.`ProductName`', 4 => '`order_details`.`UnitPrice`', 5 => '`order_details`.`Quantity`', 6 => '`order_details`.`Discount`', 7 => '`order_details`.`Subtotal`'],
					'records-per-page' => 10,
					'default-sort-by' => 1,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
			...
			'products' => [
				'SupplierID' => [
					...
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => [1 => 'Product Name', 2 => 'Supplier', 3 => 'Category', 4 => 'Quantity Per Unit', 5 => 'Unit Price', 9 => 'Discontinued', 10 => 'Total Sales'],
					'display-field-names' => [1 => 'ProductName', 2 => 'SupplierID', 3 => 'CategoryID', 4 => 'QuantityPerUnit', 5 => 'UnitPrice', 9 => 'Discontinued', 10 => 'TotalSales'],
					'sortable-fields' => [0 => '`products`.`ProductID`', 1 => 2, 2 => '`suppliers1`.`CompanyName`', 3 => '`categories1`.`CategoryName`', 4 => 5, 5 => '`products`.`UnitPrice`', 6 => '`products`.`UnitsInStock`', 7 => '`products`.`UnitsOnOrder`', 8 => '`products`.`ReorderLevel`', 9 => '`products`.`Discontinued`', 10 => '`products`.`TotalSales`', 11 => 12],
					'records-per-page' => 10,
					'default-sort-by' => 1,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
